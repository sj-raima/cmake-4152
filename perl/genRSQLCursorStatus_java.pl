...
# This should be a definition-generating line.  Do the conversion.
    ...
    if (/\s+(\w+)\s*=\s*(\w+)/) {
        my ($type_pref, $cur_status) = split('cursorStatus', $1);
